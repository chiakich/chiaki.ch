...
const AkitraHero = () => (
  <>
    <Box pt="96px" position="relative" overflow="hidden">
      <Flex height={{ base: '52vh', md: '62vh' }} minHeight="360px">{Object.values(AKITRA_COLORS).map((color, index) => <MotionBox key={color} flex="1" style={{ backgroundColor: color }} initial={{ y: '-100%' }} animate={{ y: 0 }} transition={{ duration: .7, delay: index * .12 }} />)}</Flex>
      <MotionBox position="absolute" inset="96px 0 0" display="flex" alignItems="center" justifyContent="center" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .45 }}>
        <Box backgroundColor="#20242e" border="2px solid rgba(255,255,255,.85)" boxShadow="0 12px 48px rgba(0,0,0,.55)" px={{ base: 8, md: 14 }} py={{ base: 5, md: 8 }} textAlign="center">
          ...
          <Text mt={2} letterSpacing=".3em" fontWeight="bold" style={{ color: AKITRA_COLORS.orange }}>臺鐵客貨車表記文字</Text>
        </Box>
      </MotionBox>
    </Box>
    <Flex backgroundColor="#20242e" py={4} px={6} justifyContent="center" gap={{ base: 6, md: 12 }} flexWrap="wrap">{samples.map((sample, index) => <Span key={sample} fontFamily="akitra" fontSize={{ base: 'lg', md: '2xl' }} style={{ color: index % 3 === 0 ? AKITRA_COLORS.cream : index % 3 === 1 ? AKITRA_COLORS.orange : '#7f9be0' }}>{sample}</Span>)}</Flex>
  </>
)
