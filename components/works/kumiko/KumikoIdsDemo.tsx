...
// 拆解式與部件都是真實的 IDS 資料。
const KumikoIdsDemo = () => (
  <svg viewBox="0 0 640 320" width="100%" role="img" aria-label="以 IDS 拆解式 ⿰糹扁 由部件組出「編」字">
    <rect width="640" height="320" rx="16" fill="#17181b" />
    <text x="72" y="84" fontSize="15" fill="#8e9096">缺字</text>
    <text x="72" y="132" fontSize="34" fill="#4a4c52" fontFamily="monospace">U+7DE8</text>
    <text x="72" y="196" fontSize="15" fill="#8e9096">IDS 拆解</text>
    ...
    <motion.text x="480" y="308" fontSize="13" fill="#8e9096" textAnchor="middle" animate={{ opacity: [0, 0, 1, 1, 0] }} transition={{ duration: 6, repeat: Infinity, times: [0, .58, .68, .92, 1] }}>部件對位後再手動微調</motion.text>
  </svg>
)
