...
const KumikoPage = () => (
  <Box backgroundColor="#101114" color="white" minHeight="100vh">
    ...
    <Container maxW="1080px" px={{ base: '24px', md: '40px' }} py={{ base: 16, md: 20 }}><Stack gap={20}>
      ...
      <Box><SectionHeading en="ARCHITECTURE" accent="#ffea2f">架構決定了體驗</SectionHeading><Grid columns={{ base: 1, md: 2 }} gap={4}>{features.map(([en, title, description], index) => <MotionSection key={en} delay={index * .06}><Box backgroundColor="#18191d" borderRadius="24px" p={7} height="100%"><Text fontFamily="mono" color="#ffea2f" letterSpacing=".18em" fontSize="xs">{en}</Text><Heading mt={3} mb={3} fontSize="xl">{title}</Heading><Text lineHeight="1.8" opacity={.7}>{description}</Text></Box></MotionSection>)}</Grid></Box>
