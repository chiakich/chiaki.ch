...
const features = [
  ['ZERO INSTALL', '真正零安裝', '純前端運作，伺服器端只有代理 GitHub 的 Cloudflare Functions。開瀏覽器登入 GitHub 就能開始。'],
  ['FONT I/O', '打開任何專案', '支援 .ufo、.designspace、.glyphs、.glyphspackage 與二進位字型，或直接從 GitHub 載入 UFO。'],
  ['FONT COMPILE', '瀏覽器裡編譯字體', '以 Pyodide 執行 fontTools，字體編譯不需要本機 Python 環境。'],
...
const KumikoPage = () => (
  <Box backgroundColor="#101114" color="white" minHeight="100vh">
    ...
    <Container maxW="1080px" px={{ base: '24px', md: '40px' }} py={{ base: 16, md: 20 }}><Stack gap={20}>
      ...
      <MotionSection><SectionHeading en="IDS ENGINE" accent="#ffea2f">用部件組出缺少的字</SectionHeading><Grid columns={{ base: 1, md: 2 }} gap={8} alignItems="center"><Text lineHeight="2" opacity={.78}>內建 IDS（表意文字描述序列）拆解引擎。要補「編」，先查出拆解式 ⿰糹扁，再從字體裡既有的「糹」與「扁」部件對位組合，補字就從畫整個字變成調整部件。</Text><Box border="1px solid #2c2e33" borderRadius="12px" overflow="hidden"><KumikoIdsDemo /></Box></Grid></MotionSection>
      <Box><SectionHeading en="ARCHITECTURE" accent="#ffea2f">架構決定了體驗</SectionHeading><Grid columns={{ base: 1, md: 2 }} gap={4}>{features.map(([en, title, description], index) => <MotionSection key={en} delay={index * .06}><Box backgroundColor="#18191d" border="1px solid #2c2e33" borderLeft="3px solid #ffea2f" p={6} height="100%"><Text fontFamily="mono" color="#ffea2f" letterSpacing=".18em" fontSize="xs">{en}</Text><Heading mt={3} mb={3} fontSize="xl">{title}</Heading><Text lineHeight="1.8" opacity={.7}>{description}</Text></Box></MotionSection>)}</Grid></Box>
