...
const KumikoWorkflow = () => {
  ...
  return (
    <svg viewBox="0 0 760 180" width="100%" role="img" aria-label={t('kumikoPage.workflowAria')}>
      ...
      <m.path d="M105 90 H655" stroke="#ffea2f" strokeWidth="2" strokeDasharray="6 8" initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 1.5 }} />
      {steps.map(([number, en], index) => {
        const x = 96 + index * 188
        return (
          <m.g key={number} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .16 }}>
