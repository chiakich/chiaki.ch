  ...
  return (
    <Box position="relative" borderRadius="16px" overflow="hidden" border="1px solid rgba(255,255,255,.22)" boxShadow="0 30px 90px rgba(2,14,24,.5)">
      ...
      <HStack position="absolute" top={4} right={4} backgroundColor="rgba(10,22,31,.72)" backdropFilter="blur(12px)" borderRadius="full" p={1}>
        <Button onClick={() => setDark(false)} px={4} py={2} borderRadius="full" border="0" cursor="pointer" backgroundColor={!dark ? '#f2f8fa' : 'transparent'} color={!dark ? '#243947' : 'white'}>{t('tokyonoPage.light')}</Button>
        <Button onClick={() => setDark(true)} px={4} py={2} borderRadius="full" border="0" cursor="pointer" backgroundColor={dark ? '#243947' : 'transparent'} color="white">{t('tokyonoPage.dark')}</Button>
      </HStack>
