...
// 依照實際輸入流程逐格重現：
// 逐鍵輸入注音 → 智慧注音組字 → ↓ 開候選窗 → 按 1 選字送出。
const frames = [
  { keys: ['ㄧ', 'ㄢ', 'ˊ'], buffer: '鹽', menu: false, committed: false, note: '逐鍵輸入注音' },
  { keys: ['ㄙ', 'ㄨ', '␣'], buffer: '鹽酥', menu: false, committed: false, note: '好打注音即時組字' },
  { keys: ['ㄐ', 'ㄧ', '␣'], buffer: '鹽酥雞', menu: false, committed: false, note: '整句一起判斷詞彙' },
  { keys: ['↓'], buffer: '鹽酥雞', menu: true, committed: false, note: '按 ↓ 開啟候選窗' },
  { keys: ['1'], buffer: '鹽酥雞', menu: false, committed: true, note: '按數字鍵選字' },
]
...
const TypingDemo = () => {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setFrame((value) => (value + 1) % frames.length), 1600)
          ...
          <AnimatePresence mode="popLayout">
            {keys.map((key, index) => (
              <motion.div key={`${frame}-${key}-${index}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: index * .18, duration: .22 }}>
                <Kbd display="inline-flex" minWidth="30px" height="30px" alignItems="center" justifyContent="center" px={2} borderRadius="7px" border="1px solid rgba(255,255,255,.3)" borderBottomWidth="3px" backgroundColor="#2b2b31" fontSize="sm" fontWeight="bold">{key}</Kbd>
              </motion.div>
            ))}
          </AnimatePresence>
          <Text fontSize="sm" color="#8b8b93" ml={2}>{note}</Text>
        </HStack>
        <AnimatePresence>
          {menu && (
    ...
    <Container maxW="1120px" height="100%" px={{ base: '24px', md: '40px' }} py={{ base: 14, md: 20 }} position="relative">
      <Flex direction={{ base: 'column', md: 'row' }} gap={{ base: 12, md: 8 }} alignItems="center" minHeight={{ md: '620px' }}>
        <MotionBox flex="1" initial={{ opacity: 0, x: -28 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .8 }}>
          ...
          <Text fontSize={{ base: 'lg', md: 'xl' }} lineHeight="1.9" maxW="560px" color="#eaf4ff" opacity={.88}>macOS 的繁體中文注音輸入法。以 Yahoo! 奇摩輸入法／KeyKey 的開源程式碼為基礎，保留原本的組字手感、候選窗與 bigram 引擎，在現代 macOS 上繼續維護。</Text>
