...
const ConversionScene = () => {
  ...
  <svg viewBox="0 0 640 300" width="100%" role="img" aria-label={t('chiakeyPage.scenes.conversion.ariaLabel')}>
    ...
    {/* 1. 按快捷鍵，然後鍵盤退場 */}
    <m.g animate={{ opacity: [1, 1, 0, 0] }} transition={{ duration: 8, repeat: Infinity, times: [0, .18, .26, 1] }}>
    ...
    {/* 2. 勾選啟用 */}
    <m.g animate={{ opacity: [0, 0, 1, 1] }} transition={{ duration: 8, repeat: Infinity, times: [0, .1, .16, 1] }}>
      ...
      <m.text x="352" y="115" fontSize="14" fontWeight="bold" fill="#28a04a" animate={{ opacity: [0, 0, 1, 1] }} transition={{ duration: 8, repeat: Infinity, times: [0, .16, .2, 1] }}>✓</m.text>
    </m.g>

    {/* 3. 示範打字：組字中還是繁體 */}
    <m.g animate={{ opacity: [0, 0, 1, 1, 0, 0] }} transition={{ duration: 8, repeat: Infinity, times: [0, .3, .38, .62, .68, 1] }}>
    ...
    <m.g animate={{ opacity: [0, 0, 1, 1, 0] }} transition={{ duration: 8, repeat: Infinity, times: [0, .5, .56, .64, .7] }}>
      <Keycap x={430} y={160} width={64} label="enter" active={false} />
    </m.g>
    {/* 4. 送出的瞬間轉成簡體（底線消失） */}
    <m.g animate={{ opacity: [0, 0, 1, 1] }} transition={{ duration: 8, repeat: Infinity, times: [0, .66, .74, 1] }}>
