...
const SegmentationScene = () => {
  ...
  <svg viewBox="0 0 640 300" width="100%" role="img" aria-label={t('chiakeyPage.scenes.segmentation.ariaLabel')}>
    ...
    {/* 斷詞前：查｜相干｜純 三段底線 */}
    <m.g animate={{ opacity: [1, 1, 0, 0] }} transition={{ duration: 7, repeat: Infinity, times: [0, stepTimes.swap, stepTimes.swap + .06, 1] }}>
      ...
      <m.line y1="112" y2="150" stroke="#241533" strokeWidth="2.5"
    ...
    {/* 斷詞後：茶香｜甘醇 兩段底線 */}
    <m.g animate={{ opacity: [0, 0, 1, 1] }} transition={{ duration: 7, repeat: Infinity, times: [0, stepTimes.swap, stepTimes.swap + .08, 1] }}>
    ...
    {/* 按鍵：← ← 然後 tab */}
    <m.g animate={{ opacity: [1, 1, 0, 0], y: [0, 0, 0, 0] }} transition={{ duration: 7, repeat: Infinity, times: [0, stepTimes.tab - .06, stepTimes.tab - .02, 1] }}>
      <m.g animate={{ y: [0, 3, 0, 3, 0, 0] }} transition={{ duration: 7, repeat: Infinity, times: [stepTimes.jump1 - .04, stepTimes.jump1 - .02, stepTimes.jump2 - .06, stepTimes.jump2 - .02, stepTimes.jump2, 1] }}>
    ...
    <m.g animate={{ opacity: [0, 0, 1, 1, 0] }} transition={{ duration: 7, repeat: Infinity, times: [0, stepTimes.tab - .06, stepTimes.tab, stepTimes.swap + .04, stepTimes.swap + .1] }}>
      <Keycap x={104} y={196} width={56} label="tab ⇥" />
    </m.g>
    <m.text x="240" y="220" fontSize="13" fill="#7c6b90" animate={{ opacity: [0, 0, 1, 1] }} transition={{ duration: 7, repeat: Infinity, times: [0, stepTimes.swap + .06, stepTimes.swap + .14, 1] }}>{t('chiakeyPage.scenes.segmentation.note')}</m.text>
  </svg>
  )
}
